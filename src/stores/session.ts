...
export const useSession = create<SessionState>()((set, get) => ({
  ...
  rate: async (rating) => {
    ...
    // ---- 重学阶段：回忆失败后重新学习，评 3/4 掌握即通过（不再二次回忆确认），1/2 回炉 ----
    if (s.phase === 'relearn') {
      ...
      try {
        ...
        set({
          ...
          status: nextIndex >= s.relearnQueue.length ? 'finished' : 'running',
        });
      } catch (e) {
    ...
    try {
      ...
      // - 复习模式：1（忘记）→ 回炉当场再考（FSRS 已把下次到期排到第二天以后）
      // - 其余评分即完成（FSRS 自行决定下次间隔）
      const recycle = (s.mode === 'learn' && rating < 3) || (s.mode === 'review' && rating === 1);
      if (recycle) {
        set({
  ...
  confirmRecall: async (remembered) => {
    ...
    if (!remembered) {
      // 没想起来：清空该词学习数据（删卡），进入重学队列重新来过（不影响其他词）
      try {
    ...
    // 记起来了：确认掌握，进入下一项（全部确认完 → 有重学词则重学，否则完成）
  ...
  skip: () => {
    ...
    if (s.status !== 'running') return;
    if (s.phase === 'recall') {
      void s.confirmRecall(true); // 跳过 = 视为记起来了
      return;
    }
    if (s.phase === 'relearn') {
      const nextIndex = s.relearnIndex + 1;
